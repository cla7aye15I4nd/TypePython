    ...
    match op {
        ...
        BinaryOp::Add => match &rhs.ty() {
            ...
            PyType::Float => {
                let lhs_float = cg
                    ...
                    .unwrap();
                super::float_ops::binary_op(&PyValue::float(lhs_float), cg, op, rhs)
            }
        ...
        BinaryOp::Mul => match &rhs.ty() {
            ...
            PyType::Bytes => {
                ...
                Ok(PyValue::new(result.into(), PyType::Bytes, None))
            }
...
pub fn unary_op<'ctx>(
    ...
    match op {
        UnaryOp::Neg => Ok(PyValue::int(
            cg.builder.build_int_neg(int_val, "neg").unwrap(),
        )),
        UnaryOp::Pos => Ok(PyValue::int(val.runtime_value().into_int_value())),
        UnaryOp::Not => {
        ...
        UnaryOp::BitNot => Ok(PyValue::int(
            cg.builder.build_not(int_val, "bitnot").unwrap(),
        )),
    }
}
